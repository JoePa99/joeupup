...
serve(async (req) => {
  ...
  try {
    ...
    if (!parameters.deal_id || !parameters.properties) {
      return new Response(JSON.stringify({ error: 'deal_id and properties are required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (response.error) {
      return new Response(JSON.stringify({ 
        error: `HubSpot deal update failed: ${response.error.message}` 
      }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
