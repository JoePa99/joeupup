...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Processing documents for OpenAI assistants:', body);

    const { document_id, agent_ids, content, metadata } = body;
    ...
    console.log('Processing document for agents:', agent_ids);
    ...
    for (const agentId of agent_ids) {
      try {
        ...
        if (!agent.vector_store_id || !agent.assistant_id) {
          console.error('Agent missing vector store or assistant ID:', agentId);
          results.push({ 
            agent_id: agentId, 
            success: false, 
            error: 'Agent has no vector store or assistant configured' 
          });
          continue;
        }
        ...
        console.log('File added to vector store for agent:', agent.name, 'File ID:', vectorStoreFile.id);

        // Create agent_documents relationship with OpenAI file IDs
