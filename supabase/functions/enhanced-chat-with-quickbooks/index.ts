...
// QuickBooks tools definitions (same as quickbooks-agent-tools)
...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch conversation history for context
    const conversationHistory = await fetchConversationHistory(supabaseClient, conversationId);

    // Prepare messages for OpenAI
    const messages = [
      {
        role: "system",
        content: `You are an AI assistant with access to QuickBooks Online data. You can help users with:
- Searching and managing customers
...
Always be helpful, accurate, and professional when discussing financial information. Use the available tools to fetch real-time data from QuickBooks when needed.`
      },
      ...conversationHistory,
      {
        role: "user",
    ...
    console.error('Enhanced chat with QuickBooks error:', error);
