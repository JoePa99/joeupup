...
serve(async (req) => {
  ...
  try {
    ...
    if (!parameters.contact_id || !parameters.properties) {
      return new Response(JSON.stringify({ error: 'contact_id and properties are required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (response.error) {
      return new Response(JSON.stringify({ 
        error: `HubSpot contact update failed: ${response.error.message}` 
      }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
