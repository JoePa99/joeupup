...
serve(async (req) => {
  ...
  try {
    ...
    const calendarUrl = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(eventId)}?${params.toString()}`;
    ...
    if (!calendarResponse.ok) {
      ...
      if (profile?.company_id) {
        await logApiCall(supabaseClient, user.id, profile.company_id, 'events', 'DELETE', calendarResponse.status, responseTime, error);
      }
    ...
    if (profile?.company_id) {
      await logApiCall(supabaseClient, user.id, profile.company_id, 'events', 'DELETE', 204, responseTime);
    }
