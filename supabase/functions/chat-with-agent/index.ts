...
async function processUserMessage(
  ...
  // Step 4.5: Build dynamic context (CompanyOS + knowledge base + playbooks)
  ...
  const expandedQueries = expandedQuery.expanded && expandedQuery.expanded.length > 0
    ...
    : [expandedQuery.original || parsedQuery || message];
  ...
  const systemPrompt = buildAssistantPrompt({
    agent: {
      ...
      personaInstructions: agentData.configuration?.instructions,
  ...
  return {
    ...
    analysis: {
      ...
      context_used: dynamicContext.contextUsed || !!toolContext
    },
