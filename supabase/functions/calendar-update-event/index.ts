...
serve(async (req) => {
  ...
  try {
    ...
    // Build event object with only provided fields and normalized dates
    const event: any = {};
    if (summary !== undefined) event.summary = summary;
    if (description !== undefined) event.description = description;
    if (start !== undefined) event.start = normalizeDateTime(start);
    if (end !== undefined) event.end = normalizeDateTime(end);
    if (location !== undefined) event.location = location;
    if (attendees !== undefined) event.attendees = attendees;
    if (conferenceData !== undefined) event.conferenceData = conferenceData;
    ...
    if (!calendarResponse.ok) {
      ...
      if (profile?.company_id) {
        await logApiCall(supabaseClient, user.id, profile.company_id, 'events', 'PATCH', calendarResponse.status, responseTime, error);
      }
    ...
    if (profile?.company_id) {
      await logApiCall(supabaseClient, user.id, profile.company_id, 'events', 'PATCH', 200, responseTime);
    }
