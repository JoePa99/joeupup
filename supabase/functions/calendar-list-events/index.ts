...
serve(async (req) => {
  ...
  try {
    ...
    const calendarUrl = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events?${params.toString()}`;
    ...
    if (!calendarResponse.ok) {
      ...
      if (profile?.company_id) {
        await logApiCall(supabaseClient, user.id, profile.company_id, 'events', 'GET', calendarResponse.status, responseTime, error);
      }
    ...
    if (profile?.company_id) {
      await logApiCall(supabaseClient, user.id, profile.company_id, 'events', 'GET', 200, responseTime);
    }
